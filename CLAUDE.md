...
- Post-ship (Previously undocumented features found in code, not in this changelog — documented 2026-09-21): `api/routes/billing.py` (`/api/billing`) — invoice CRUD, PDF generation/email, Stripe payment links, `POST /api/billing/stripe/webhook`; `api/routes/psa.py` (`/api/psa`) — ConnectWise Manage / Autotask PSA integration CRUD, connection test, company mapping, synced every 15min by `api/tasks/psa_tasks.py::sync_all_psa_integrations`; `api/routes/update.py` (`/api/agents/update`) — agent auto-update (`/info`, `/check`, `/download` streams a SHA-256-verified signed binary, gated by `LATEST_AGENT_VERSION`/`AGENT_UPDATE_PATH`); `api/routes/org_settings.py` (`/api/admin`) — org-level settings; `api/tasks/email_tasks.py::poll_support_inbox` (60s beat) — IMAP inbox polling that auto-creates tickets from inbound email (`SUPPORT_IMAP_*` / `DEFAULT_INBOUND_CUSTOMER_ID` env vars); `api/tasks/anomaly_tasks.py::detect_metric_anomalies` (10min beat) — metric anomaly detection; `api/tasks/maintenance_tasks.py::prune_old_data` (daily beat); `api/tasks/ticket_tasks.py::check_sla_breaches` (hourly beat); `api/tasks/alert_tasks.py::unlock_expired_accounts` (60s beat), `::deactivate_dormant_accounts` (daily beat, distinct from the alert auto-resolve logic above), `::check_password_expiry` (daily beat); TOTP-based MFA (per README, not previously listed here). See `.env.example` for the full env-var surface — it was ahead of this changelog.
- Post-ship (Production-readiness diagnostic — 2026-09-21): Fixed `api/app.py` defaulting to Werkzeug debug mode (`debug=True`, interactive debugger, bound `0.0.0.0`) whenever `FLASK_DEBUG` was unset — default flipped from `"1"` to `"0"`; `docker-compose.yml` now sets `FLASK_DEBUG: "0"` explicitly on the `api` service and requires `DB_PASSWORD` (was `${DB_PASSWORD:-changeme}`, silently insecure if unset — now `${DB_PASSWORD:?...}`, fails closed); Postgres/Redis container ports now bind to `127.0.0.1` only instead of `0.0.0.0` (Redis has no `requirepass` configured, so it was reachable unauthenticated from any host that could reach the machine); `.env.example` gained `DB_PASSWORD` (previously undocumented, only consumed by `docker-compose.yml`) and a warning comment on `FLASK_DEBUG`. **Removed real secrets that were committed to git**: `agent/config.ini` (contained a live `device_id` + DPAPI-encrypted `agent_token` for an already-registered device) is now gitignored — untracked from the index but left on disk; added `agent/config.ini.example` as the template. `agent/install.bat` and `.claude/state.md` had the real `ORG_REGISTRATION_TOKEN` value hardcoded in plaintext — both scrubbed (`install.bat` now prompts for `RMM_SERVER_IP`/`RMM_ORG_TOKEN` instead of hardcoding them). **These values are still present in git history** (pushed to `origin/main`) — rotate `ORG_REGISTRATION_TOKEN` in `.env` and re-issue via Admin panel; consider `git filter-repo`/BFG + force-push if repo visibility is a concern. Also removed from git tracking (kept locally, now gitignored): `chunk.txt`, `debug.txt`, `dump.rdb`/`api/dump.rdb`, `redis.zip` + vendored `redis/` binary directory (47MB, unreferenced by any documented workflow — install Redis/Memurai separately), `patch_handover*.py`/`patch_technical*.py` (one-off doc-patching scratch scripts), `generate_pdf.py` (redundant duplicate of `build_pdf.py`, which is the maintained HANDOVER_GUIDE.md→PDF tool and was kept), `api/celerybeat-schedule-shm` (Celery runtime artifact). Corrected page counts documented elsewhere in this file and in README.md: Streamlit has **22** pages (not 19) — `09_Billing`, `10_Invoice_Detail`, `14_Disk_Management`, `20_Client_Portal`, `21_Client_Tickets` were never counted; the React frontend has **18** pages and is **not** at parity with Streamlit as previously claimed — missing App Center, Invoice Detail, IoT Sensors, and Client Tickets equivalents (known gap, not fixed in this pass). Test suite: 106/106 passing (`cd api && python -m pytest tests -q`, SQLite in-memory, no live DB/Redis needed) — note the environment was missing `pytest`, `pyotp`, `stripe`, `python-dotenv` despite them being in `api/requirements.txt`; reinstall requirements if you hit `ModuleNotFoundError` running tests locally.
- Post-ship (Production-readiness diagnostic, follow-up — 2026-09-21): Rotated the leaked `ORG_REGISTRATION_TOKEN` in local `.env` (safe — already-registered agents use their own per-device `agent_token`, not the org token, so no re-registration needed; only pending/new agent enrollments need the new value). `docker-compose.yml` Redis now runs with `--requirepass ${REDIS_PASSWORD:?...}` and the `api`/`celery_worker`/`celery_beat` services build authenticated `redis://:${REDIS_PASSWORD}@redis:6379/N` URLs — no code changes needed since `api/utils/cache.py`, `tasks/celery_app.py`, and `tasks/alert_tasks.py`'s distributed lock all already read the URL from env via `redis.from_url()`. `.env.example` gained `REDIS_PASSWORD` (docker-compose only; not required for local non-Docker dev). Still open, deliberately not done: scrubbing the leaked token/device credentials from git history (destructive — force-push, breaks other clones — needs an explicit decision) and closing the React/Streamlit page-parity gap (a real feature-build task, not a quick fix).
...
- `api/routes/billing.py` — invoice CRUD, PDF generation/email, Stripe payment links, `POST /api/billing/stripe/webhook`. Needs `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`.
- `api/routes/psa.py` + `api/tasks/psa_tasks.py` — ConnectWise Manage / Autotask PSA integration CRUD + 15-min sync beat task. Credentials stored encrypted in DB, configured via Admin → PSA Integrations.
- `api/routes/update.py` — agent auto-update (`/api/agents/update/info|check|download`). Gated by `LATEST_AGENT_VERSION`/`AGENT_UPDATE_PATH` env vars; download is SHA-256 verified.
- `api/routes/org_settings.py` — org-level settings under `/api/admin`.
- `api/tasks/email_tasks.py` — `poll_support_inbox` (60s beat) polls `SUPPORT_IMAP_*` inbox and auto-creates tickets; unknown senders fall back to `DEFAULT_INBOUND_CUSTOMER_ID` or are skipped.
- `api/tasks/anomaly_tasks.py` — `detect_metric_anomalies` (10min beat) flags abnormal device metrics.
- `agent/config.ini` — gitignored (contains a live `device_id` + DPAPI-encrypted `agent_token`); copy `agent/config.ini.example` to `agent/config.ini` for a new deployment instead of editing a committed copy.
